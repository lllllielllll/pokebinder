...
export default function AdicionarCartaPage() {
  ...
  try {
    ...
    let cards = detailedCards

    if (setFilter.trim()) {
      cards = cards.filter((card: ApiCard) =>
        card.set?.name?.toLowerCase().includes(setFilter.toLowerCase())
      )
    }
    ...
    if (numberFilter.trim()) {
      ...
      cards = cards.filter((card: ApiCard) => {
        const apiNumber = String(card.localId || card.localId || '')
          .split('/')[0]
          .replace(/^0+/, '')
          .toLowerCase()
